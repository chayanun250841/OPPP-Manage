...
**ด่านกันเสกเงิน:** ทุกรายการปรับต้องมีผลรวม `delta x rate_claim` = 0 บาท (เพิ่มบริการหนึ่ง ต้องหักบริการอื่นให้เงินเท่าเดิม) `_load_adjustments()` จะทิ้งรายการที่ไม่สมดุล/ใช้รหัสที่ไม่มีจริง และ `_apply_adjustments()` จะทิ้งรายการที่หักจนจำนวนครั้งติดลบ พร้อมขึ้นบรรทัด ⚠️ บนตารางแทน ผลคือ `รวมจาก สปสช.` และ `ยังไม่จัดประเภท` ของทุกหน่วยจะไม่มีวันขยับจากการปรับด้วยมือ (ทดสอบครบ 4 เคสแล้ว) ส่วน `จัดสรรตามมติจังหวัด` ขยับได้ เพราะอัตราส่วนแบ่งจังหวัดของแต่ละรายการไม่เท่ากัน

**การแสดงผล:** ตารางบนหน้าเว็บแสดงจำนวนครั้งที่ปรับแล้วเฉยๆ ไม่มีบรรทัดกำกับ (ผู้ใช้สั่งไว้ 2026-08-17 — เดิมมีบรรทัด `✏️ ปรับด้วยมือ` ต่อท้าย) แปลว่า **ดูจากหน้าเว็บอย่างเดียวจะแยกไม่ออกว่าตัวเลขไหนมาจากยอดเงินจริงและตัวเลขไหนถูกปรับ** ต้องเปิด `assets/manual_adjustments.json` (หรือ git log ของแฟ้มนี้) เท่านั้น ใครแตะรายงานชุดนี้ต่อควรรู้ข้อนี้ก่อน ส่วนรายการที่ระบบปฏิเสธยังขึ้นคำเตือน ⚠️ บนตารางเหมือนเดิม เพราะเป็นความผิดพลาดของแฟ้มที่ต้องมีคนแก้ ไม่ใช่ผลการปรับ
