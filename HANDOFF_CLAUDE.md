...
### การปรับจำนวนครั้งด้วยมือรายหน่วยบริการ (เพิ่ม 2026-08-17)

`assets/manual_adjustments.json` — ใช้เมื่อหน่วยบริการยืนยันว่าให้บริการจริง แต่ยอดเงินที่ สปสช. ส่งมาไม่มีก้อนที่ระบบจับคู่เข้ารายการนั้นได้ `service_analysis.summarize_item_counts(predictions, hcode)` จะบวก/ลบจำนวนครั้งตามแฟ้มนี้หลังนับจากยอดเงินเสร็จแล้ว มีผลทั้งแท็บรายหน่วยและตารางสรุปทุกหน่วยบริการ

**ด่านกันเสกเงิน:** ทุกรายการปรับต้องมีผลรวม `delta x rate_claim` = 0 บาท (เพิ่มบริการหนึ่ง ต้องหักบริการอื่นให้เงินเท่าเดิม) `_load_adjustments()` จะทิ้งรายการที่ไม่สมดุล/ใช้รหัสที่ไม่มีจริง และ `_apply_adjustments()` จะทิ้งรายการที่หักจนจำนวนครั้งติดลบ ผลคือ `รวมจาก สปสช.` และ `ยังไม่จัดประเภท` ของทุกหน่วยจะไม่มีวันขยับจากการปรับด้วยมือ (ทดสอบครบ 4 เคสแล้ว) ส่วน `จัดสรรตามมติจังหวัด` ขยับได้ เพราะอัตราส่วนแบ่งจังหวัดของแต่ละรายการไม่เท่ากัน

**การแสดงผล — หน้าเว็บเงียบสนิท** (ผู้ใช้สั่งไว้ 2026-08-17 เดิมมีบรรทัด `✏️ ปรับด้วยมือ` และคำเตือน ⚠️ บนตาราง ตอนนี้ตัดออกทั้งคู่):

- ตารางแสดงจำนวนครั้งที่ปรับแล้วปนกับที่จับคู่จากยอดเงินจริง **แยกไม่ออกจากหน้าเว็บ** ต้องเปิด `assets/manual_adjustments.json` หรือ `git log -p assets/manual_adjustments.json` เท่านั้น แฟ้มนั้นคือหลักฐานชิ้นเดียวที่เหลืออยู่ ห้ามลบ
- รายการที่ถูกปฏิเสธก็เงียบเหมือนกัน **อาการคือแก้ JSON แล้วตัวเลขไม่ขยับเฉยๆ** ให้ดูสาเหตุที่ Render → Logs บรรทัด `ข้ามการปรับด้วยมือของ <hcode>` (ออกทาง `logging.warning` ของ `service_analysis`)

รายการที่บันทึกไว้ตอนนี้ — **07846 รพ.สต.นายาว**: ANC ทันตกรรม +3 (1,500 บาท) หักคืนตามสัดส่วนเงินจาก ANC ฝากครรภ์ −2 (−720), ยาคุม COC −4 (−480), ยาฉีดคุมกำเนิด −5 (−300) เหตุเพราะข้อมูลดิบปีงบ 2569 ของนายาวไม่มียอด 500 บาทเลยสักแถว (ยอดที่มี: 60×40, 120×31, 360×11, 435×2, 135, 75, 150, 345) จึงเป็น override ตามที่ผู้ใช้สั่ง ไม่ใช่การแก้ให้ตรงต้นทาง — ถ้ามีเอกสารยืนยันการให้บริการ ควรแนบไว้กับแฟ้มนี้
